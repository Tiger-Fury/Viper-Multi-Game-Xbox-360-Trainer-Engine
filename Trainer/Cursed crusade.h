...
float CursedCXValue = 10000.0;
//float ZValue = 0.0;


VOID __declspec(naked) CursedCCheckHealth(VOID)
{ 
...
VOID CursedCSetHooks()
{
    Caveme(0x8249C82C,CursedCCheckHealth,15);
    HooksSet = true;
}

VOID Trainer445407DE()
{
	Prompt = XShowMessageBoxUI(0, L"~TeamXPG~ t3fury XPGamesaves.com", L"Cursed Crusade + 5\r\n~~~~~~~~~~~~~~~~~~~~~~~~~\r\nView Available Codes: DPAD-DOWN(x4)\r\nActivate All Codes: DPAD-UP(x4)\r\n", ARRAYSIZE(g_strButtons), g_strButtons, 0, XMB_PASSCODEMODE, &g_mb_result, &g_xol);
	if( HooksSet == false )
	{
		CursedCSetHooks();
	}
...
VOID Press445407DE( MESSAGEBOX_RESULT g_mb_result )
{
...
// 	}
	if( g_mb_result.rgwPasscode[0] == LEFT_TRIGGER && g_mb_result.rgwPasscode[1] == LEFT_TRIGGER && g_mb_result.rgwPasscode[2] == LEFT_TRIGGER && g_mb_result.rgwPasscode[3] == LEFT_TRIGGER )
	{
		...
		if( InfAmmo == TRUE )
		{
			...
			SetDWORD(0x82291A7C,0x914B00E4);
			InfAmmo = FALSE;
			Vibr01();
			...
			SetDWORD(0x82291A7C, 0x396003E7);
			InfAmmo = TRUE;
			Vibr01();
...
VOID Boot_445407DE()
{
	...
	Sleep(25);
	DWORD CursedCuv_buf[] = {0x396BFFFF, 0x91631120};
	uv_addr = GetAddr(0x82100000, 0x83000000, CursedCuv_buf, 2, 1);
	DWORD CursedCCharge_buf[] = {0x394AFFFF, 0x914B1124};
	Charge_addr = GetAddr(0x82100000, 0x83000000, CursedCCharge_buf, 2, 1);
	SetAddr = true;
}
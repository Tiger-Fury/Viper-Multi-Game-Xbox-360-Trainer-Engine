...
DWORD addr_ammoSHHC = 0x82270AD0;
DWORD Ammo_OnSHHC = 0x396003E7;
DWORD Ammo_offSHHC = 0x7D6A5850;
float SHHCXValue = 200.0;
float SHHCZValue = 0.0;


VOID __declspec(naked) SHHCCheckHealth(VOID)
{ 
...
VOID SHHCSetHooks()
{
    Caveme(0x822bcaf0,SHHCCheckHealth,8);
    HooksSet = true;
}

VOID Trainer4B4E07E5()
{
	Prompt = XShowMessageBoxUI(0, L"https://www.t3fury.co.uk", L"Silent Hill HomeComing + 3\r\n~~~~~~~~~~~~~~~~~~~~~~~~~\r\nView Available Codes: DPAD-DOWN(x4)\r\nActivate All Codes: DPAD-UP(x4)\r\n", ARRAYSIZE(g_strButtons), g_strButtons, 0, XMB_PASSCODEMODE, &g_mb_result, &g_xol);
	if( HooksSet == false )
	{
		SHHCSetHooks();
	}
...
VOID Press4B4E07E5( MESSAGEBOX_RESULT g_mb_result )
{
	...
	if( g_mb_result.rgwPasscode[0] == LEFT_TRIGGER && g_mb_result.rgwPasscode[1] == LEFT_TRIGGER && g_mb_result.rgwPasscode[2] == LEFT_TRIGGER && g_mb_result.rgwPasscode[3] == LEFT_TRIGGER )
	{
		...
		if( InfAmmo == TRUE )
		{
			...
			SetDWORD(addr_ammoSHHC,Ammo_offSHHC);
			InfAmmo = FALSE;
			Vibr01();
			...
			SetDWORD(addr_ammoSHHC, Ammo_OnSHHC);
			InfAmmo = TRUE;
			Vibr01();
...
VOID Boot_4B4E07E5()
{
	...
	Run_Input = Press4B4E07E5;
	Run_Trainer = Trainer4B4E07E5;
